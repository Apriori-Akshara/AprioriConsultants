...
const rwBucket = (q) => q.skill === 'Cross-Text Connections' ? 'relationship-first cross-text' : q.skill === 'Rhetorical Synthesis' ? 'notes → communication goal → synthesis' : q.skill === 'Words in Context' ? 'contextual lexical inference' : 'reasoning-demand-driven R&W construction';
const mathBucket = (q) => q.figure ? 'figure' : 'strategic';
function assertUnique(items, label) { const prompts = items.map((item) => normalize(item.prompt)); const unique = new Set(prompts); if (unique.size !== prompts.length) throw new Error(`${label}: duplicate normalized prompts (${prompts.length - unique.size})`); }
function runVariant(product) {
  ...
  for (const [bucket, required] of Object.entries(REQUIREMENTS[product].rw)) { if (rwGroups[bucket].length < required) throw new Error(`${product} R&W ${bucket}: need ${required}, have ${rwGroups[bucket].length}`); assertUnique(rwGroups[bucket].slice(0, required), `${product} R&W ${bucket}`); }
  ...
  const mathGroups = { strategic: mathPool.candidates.filter((q) => mathBucket(q) === 'strategic'), figure: mathPool.candidates.filter((q) => mathBucket(q) === 'figure') };
  for (const [bucket, required] of Object.entries(REQUIREMENTS[product].math)) { if (mathGroups[bucket].length < required) throw new Error(`${product} Math ${bucket}: need ${required}, have ${mathGroups[bucket].length}`); assertUnique(mathGroups[bucket].slice(0, required), `${product} Math ${bucket}`); }
  ...
  return { rwPool: rwPool.candidates.length, rwUnique: Object.fromEntries(Object.entries(rwGroups).map(([k, v]) => [k, new Set(v.map((q) => normalize(q.prompt))).size])), rwQuality, mathPool: mathPool.candidates.length, mathUnique: Object.fromEntries(Object.entries(mathGroups).map(([k, v]) => [k, new Set(v.map((q) => normalize(q.prompt))).size])), mathQuality, sprPct };
}
