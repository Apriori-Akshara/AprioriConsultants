...
const EXPECTED_KEYS = new Set(Array.from({ length: 10 }, (_, i) => `SAT${i + 1}`).concat(Array.from({ length: 10 }, (_, i) => `PSAT${i + 1}`)));
...
for (const [key, replacement] of Object.entries(BATCH_M_CONTROLLED_REPLACEMENT_MAP)) {
  ...
  addCount(changedByMock, testKey);
}
...
for (const key of postQuestions.keys()) {
  const testKey = key.split('::')[0];
  if (!EXPECTED_KEYS.has(testKey)) fail('out-of-scope-runtime-key', key);
}
...
for (const [key, question] of postQuestions) {
  ...
  if (!schema.valid) {
    schemaFailures += 1;
    fail('schema', { key, errors: schema.errors });
  }

  const quality = evaluateContentQuality(question);
  if (quality.verdict !== 'pass') {
    qualityFailures += 1;
    for (const reason of quality.checks) addCount(failureReasonCounts, reason);
  }
  const mockSummary = qualityByMock.get(testKey) || { total: 0, passed: 0, failed: 0, scoreSum: 0 };
  mockSummary.total += 1;
  mockSummary.scoreSum += quality.score;
  if (quality.verdict === 'pass') mockSummary.passed += 1;
  else mockSummary.failed += 1;
  qualityByMock.set(testKey, mockSummary);
...
for (const [testKey, summary] of qualityByMock) {
  if (summary.failed > 0) fail('content-quality', `${testKey}: ${summary.failed}/${summary.total} questions failed; average score ${(summary.scoreSum / summary.total).toFixed(2)}`);
}

try {
  validateFigureOriginalitySeries(appliedCorpus);
} catch (error) {
  fail('figure-originality', String(error?.message || error));
}
...
const report = {
  ...
  sat21Created: false,
  expected: {
    affectedRecords: EXPECTED_AFFECTED,
    mocks: EXPECTED_MOCKS,
    recordsPerMock: EXPECTED_RECORDS_PER_MOCK,
    runtimeQuestions: EXPECTED_RUNTIME_QUESTIONS,
    replacements: EXPECTED_REPLACEMENTS,
  },
  observed: {
    preReplacementRuntimeQuestions: preQuestions.size,
    postReplacementRuntimeQuestions: postQuestions.size,
    mocks: appliedCorpus.length,
    replacementMapEntries: Object.keys(BATCH_M_CONTROLLED_REPLACEMENT_MAP).length,
    changedReplacementTargets: changedCount,
    schemaFailures,
    contentQualityFailures: qualityFailures,
    figureQuestionsWithDeterministicFingerprint: figureFingerprintCount,
  },
  gates: {
...
if (!report.passed) {
  console.error(`Batch M comprehensive 20-test QC FAILED with ${failures.length} gate failures.`);
  process.exitCode = 1;
} else {
  console.log('Batch M comprehensive 20-test QC PASSED.');
}
