...
const difficultyLine = "  let difficulty = TARGETED_GEOMETRY_DIFFICULTY_CYCLE[o % TARGETED_GEOMETRY_DIFFICULTY_CYCLE.length];";
...
if (!source.includes(difficultyLine)) throw new Error('Geometry difficulty cycle line is missing.');
...
source = source.replace(
  "const suffix = 'geometry-difficulty-v4-' + skill.replace(/[^A-Za-z0-9]+/g, '-').toLowerCase()",
  "const suffix = 'geometry-difficulty-v7-' + targetedSkill.replace(/[^A-Za-z0-9]+/g, '-').toLowerCase()",
);
source = source.replace(
