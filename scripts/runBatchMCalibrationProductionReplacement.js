...
function getRuntimeMock(target) {
  ...
  return SERIES_A[target.testKey] || null;
}
...
function main() {
  ...
  fs.writeFileSync(`${OUTPUT_DIR}/BATCH-M-CALIBRATION-PRODUCTION-REPLACEMENT-2026-09-17.md`, [
    ...
    '- Status: **PASS**.', '- Production replacement layer: **ACTIVE**.', '- Authorized replacements: **352**.',
    '- R&W replacements: **195** (65 Craft & Structure → SEC; 130 Information & Ideas → SEC).',
    '- Math replacements: **157 SPR**.', '- Final 30-mock corpus gate: **PASS**.', '- Cross-corpus calibration: **PASS**.',
    '- Release eligibility: **false** (post-gate website verification remains required).', '- SAT21 created: **false**.',
  ].join('\n') + '\n');
