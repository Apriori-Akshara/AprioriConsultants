...
function assertCrossMockUniqueness(previousMock, nextMock) {
  const previousRWContexts = new Set((previousMock.readingWriting || []).map((q) => String(q.metadata?.contextKey || '').trim().toLowerCase()).filter(Boolean));
  const previousRWPrompts = new Set((previousMock.readingWriting || []).map((q) => String(q.prompt || '').trim().toLowerCase().replace(/\s+/g, ' ')).filter(Boolean));
  const previousApplications = new Set((previousMock.math || []).map((q) => String(q.metadata?.applicationFingerprint || '').trim().toLowerCase()).filter(Boolean));
  ...
  for (const question of nextMock.readingWriting || []) {
    ...
    if (context && previousRWContexts.has(context)) throw new Error(`Batch M SAT2: R&W context reused from SAT Series A Mock 1: ${question.questionId}`);
    if (prompt && previousRWPrompts.has(prompt)) throw new Error(`Batch M SAT2: R&W prompt reused from SAT Series A Mock 1: ${question.questionId}`);
  }

  for (const question of nextMock.math || []) {
    const application = String(question.metadata?.applicationFingerprint || '').trim().toLowerCase();
    if (application && previousApplications.has(application)) throw new Error(`Batch M SAT2: Math application reused from SAT Series A Mock 1: ${question.questionId}`);
  }

  validateFigureOriginalitySeries([previousMock, nextMock]);
  const previousFigureData = new Set((previousMock.math || []).map(getFigureDataFingerprint).filter(Boolean));
  for (const question of nextMock.math || []) {
    const fingerprint = getFigureDataFingerprint(question);
    if (fingerprint && previousFigureData.has(fingerprint)) throw new Error(`Batch M SAT2: figure data reused from SAT Series A Mock 1: ${question.questionId}`);
  }
}

function assertCanonicalRoundTrip(mock) {
  const records = [...(mock.readingWriting || []), ...(mock.math || [])];
  if (records.length !== 196) throw new Error(`Batch M SAT2: expected 196 records, found ${records.length}`);
  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) throw new Error(`Batch M SAT2: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
  }
  const restored = JSON.parse(JSON.stringify(records));
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) throw new Error('Batch M SAT2: storage round-trip changed question IDs');
  return restored;
}

export function runBatchMSecondProductionGate(previousMock) {
  if (!TARGET) throw new Error('Batch M SAT2: production target is missing');
  assertBatchMProductionOrder(['SAT1']);
  if (!previousMock || previousMock.testId !== 'sat-series-a-mock-01') throw new Error('Batch M SAT2: accepted Mock 1 baseline is required');

  const stage1 = buildProductionMock({ testId: 'sat-mock-02', variant: 'sat-series-a', seed: 1002 });
  const withMath = { ...stage1, math: buildMathBank({ testId: 'sat-mock-02', variant: 'sat-series-a', assessmentNumber: 2, seed: 1002 }) };
  ...
  assertCrossMockUniqueness(previousMock, productionMock);
  ...
  return {
    ...
    productionMock: { ...productionMock, storageRecords: restoredRecords },
  };
}
