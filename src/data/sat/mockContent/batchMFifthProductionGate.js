/** Batch M — SAT Series A Mock 5 production gate. */
...
const TARGET = getBatchMProductionTarget('SAT5');
...
function assertCrossMockUniqueness(previousMocks, nextMock) {
  ...
  for (const question of nextMock.readingWriting || []) {
    const context = String(question.metadata?.contextKey || '').trim().toLowerCase();
    const prompt = String(question.prompt || '').trim().toLowerCase().replace(/\s+/g, ' ');
    if (context && previousRWContexts.has(context)) throw new Error(`Batch M SAT5: R&W context reused from an accepted earlier mock: ${question.questionId}`);
    if (prompt && previousRWPrompts.has(prompt)) throw new Error(`Batch M SAT5: R&W prompt reused from an accepted earlier mock: ${question.questionId}`);
  }
  for (const question of nextMock.math || []) {
    const application = String(question.metadata?.applicationFingerprint || '').trim().toLowerCase();
    if (application && previousApplications.has(application)) throw new Error(`Batch M SAT5: Math application reused from an accepted earlier mock: ${question.questionId}`);
    const figureData = getFigureDataFingerprint(question);
    if (figureData && previousFigureData.has(figureData)) throw new Error(`Batch M SAT5: figure data reused from an accepted earlier mock: ${question.questionId}`);
  }
...
function assertCanonicalRoundTrip(mock) {
  ...
  if (records.length !== 196) throw new Error(`Batch M SAT5: expected 196 records, found ${records.length}`);
  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) throw new Error(`Batch M SAT5: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
  }
  const restored = JSON.parse(JSON.stringify(records));
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) throw new Error('Batch M SAT5: storage round-trip changed question IDs');
  return restored;
}

export function runBatchMFifthProductionGate(previousMocks) {
  if (!TARGET) throw new Error('Batch M SAT5: production target is missing');
  assertBatchMProductionOrder(['SAT1', 'SAT2', 'SAT3', 'SAT4']);
  if (!Array.isArray(previousMocks) || previousMocks.length !== 4) throw new Error('Batch M SAT5: accepted Mock 1, Mock 2, Mock 3, and Mock 4 baselines are required');
  const expectedIds = ['sat-series-a-mock-01', 'sat-series-a-mock-02', 'sat-series-a-mock-03', 'sat-series-a-mock-04'];
  if (previousMocks.some((mock, index) => mock?.testId !== expectedIds[index])) throw new Error('Batch M SAT5: accepted Series A baselines are out of order');
  const stage1 = buildProductionMock({ testId: 'sat-mock-05', variant: 'sat-series-a', seed: 1005 });
  const withMath = { ...stage1, math: buildMathBank({ testId: 'sat-mock-05', variant: 'sat-series-a', assessmentNumber: 5, seed: 1005 }) };
...
export default runBatchMFifthProductionGate;
