...
const TARGET = getBatchMProductionTarget('SAT9');
...
function assertCrossMockUniqueness(previousMocks, nextMock) {
  ...
  for (const question of nextMock.readingWriting || []) {
    const context = String(question.metadata?.contextKey || '').trim().toLowerCase();
    const prompt = String(question.prompt || '').trim().toLowerCase().replace(/\s+/g, ' ');
    if (context && previousRWContexts.has(context)) throw new Error(`Batch M SAT9: R&W context reused: ${question.questionId}`);
    if (prompt && previousRWPrompts.has(prompt)) throw new Error(`Batch M SAT9: R&W prompt reused: ${question.questionId}`);
  }
  for (const question of nextMock.math || []) {
    const application = String(question.metadata?.applicationFingerprint || '').trim().toLowerCase();
    if (application && previousApplications.has(application)) throw new Error(`Batch M SAT9: Math application reused: ${question.questionId}`);
    const figureData = getFigureDataFingerprint(question);
    if (figureData && previousFigureData.has(figureData)) throw new Error(`Batch M SAT9: figure data reused: ${question.questionId}`);
  }
...
function assertCanonicalRoundTrip(mock) {
  ...
  if (records.length !== 196) throw new Error(`Batch M SAT9: expected 196 records, found ${records.length}`);
  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) throw new Error(`Batch M SAT9: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
  }
  const restored = JSON.parse(JSON.stringify(records));
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) throw new Error('Batch M SAT9: storage round-trip changed question IDs');
  return restored;
}

export function runBatchMNinthProductionGate(previousMocks) {
  if (!TARGET) throw new Error('Batch M SAT9: production target is missing');
  assertBatchMProductionOrder(['SAT1', 'SAT2', 'SAT3', 'SAT4', 'SAT5', 'SAT6', 'SAT7', 'SAT8']);
  if (!Array.isArray(previousMocks) || previousMocks.length !== 8) throw new Error('Batch M SAT9: accepted Mock 1 through Mock 8 baselines are required');
  const expectedIds = ['sat-series-a-mock-01', 'sat-series-a-mock-02', 'sat-series-a-mock-03', 'sat-series-a-mock-04', 'sat-series-a-mock-05', 'sat-series-a-mock-06', 'sat-series-a-mock-07', 'sat-series-a-mock-08'];
  if (previousMocks.some((mock, index) => mock?.testId !== expectedIds[index])) throw new Error('Batch M SAT9: accepted Series A baselines are out of order');

  const stage1 = buildProductionMock({ testId: 'sat-mock-09', variant: 'sat-series-a', seed: 1009 });
  const withMath = { ...stage1, math: buildMathBank({ testId: 'sat-mock-09', variant: 'sat-series-a', assessmentNumber: 9, seed: 1009 }) };
...
export default runBatchMNinthProductionGate;
