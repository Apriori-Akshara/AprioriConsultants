/**
 * Batch M — SAT Series A Mock 8 production gate.
 *
 * Mock 8 is accepted only after SAT Series A Mocks 1–7 have already passed.
...
const TARGET = getBatchMProductionTarget('SAT8');
...
function assertCrossMockUniqueness(previousMocks, nextMock) {
  ...
  for (const question of nextMock.readingWriting || []) {
    ...
    if (context && previousRWContexts.has(context)) {
      throw new Error(`Batch M SAT8: R&W context reused from an accepted earlier mock: ${question.questionId}`);
    }
    if (prompt && previousRWPrompts.has(prompt)) {
      throw new Error(`Batch M SAT8: R&W prompt reused from an accepted earlier mock: ${question.questionId}`);
    }
  }
  ...
  for (const question of nextMock.math || []) {
    ...
    if (application && previousApplications.has(application)) {
      throw new Error(`Batch M SAT8: Math application reused from an accepted earlier mock: ${question.questionId}`);
    }
    const figureData = getFigureDataFingerprint(question);
    if (figureData && previousFigureData.has(figureData)) {
      throw new Error(`Batch M SAT8: figure data reused from an accepted earlier mock: ${question.questionId}`);
    }
  }
...
function assertCanonicalRoundTrip(mock) {
  ...
  if (records.length !== 196) {
    throw new Error(`Batch M SAT8: expected 196 records, found ${records.length}`);
  }

  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) {
      throw new Error(`Batch M SAT8: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
    }
  }
  ...
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) {
    throw new Error('Batch M SAT8: storage round-trip changed question IDs');
  }

  return restored;
}

export function runBatchMEighthProductionGate(previousMocks) {
  if (!TARGET) throw new Error('Batch M SAT8: production target is missing');
  assertBatchMProductionOrder(['SAT1', 'SAT2', 'SAT3', 'SAT4', 'SAT5', 'SAT6', 'SAT7']);

  if (!Array.isArray(previousMocks) || previousMocks.length !== 7) {
    throw new Error('Batch M SAT8: accepted Mock 1 through Mock 7 baselines are required');
  }
  ...
  const expectedIds = [
    ...
    'sat-series-a-mock-06',
    'sat-series-a-mock-07',
  ];
  if (previousMocks.some((mock, index) => mock?.testId !== expectedIds[index])) {
    throw new Error('Batch M SAT8: accepted Series A baselines are out of order');
  }

  const stage1 = buildProductionMock({ testId: 'sat-mock-08', variant: 'sat-series-a', seed: 1008 });
  const withMath = {
    ...stage1,
    math: buildMathBank({ testId: 'sat-mock-08', variant: 'sat-series-a', assessmentNumber: 8, seed: 1008 }),
  };
...
export default runBatchMEighthProductionGate;
