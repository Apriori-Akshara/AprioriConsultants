...
function canonicalizeFigure(question) {
  if (!question.figure) {
    const match = String(question.prompt || '').match(/linear model is y = (-?\d+(?:\.\d+)?)x \+ (-?\d+(?:\.\d+)?)/i);
    if (match && question.domain === 'Algebra' && /Linear functions and representations|Linear representations/.test(String(question.skill || ''))) {
      return { type: 'linear_function_graph', slope: Number(match[1]), y_intercept: Number(match[2]), x_range: [-10, 10] };
    }
    return null;
  }
  const figure = question.figure;
  if (figure.type === 'quadratic') {
    const a = Number(figure.a); const b = Number(figure.b); const c = Number(figure.c);
    const vertexX = a ? -b / (2 * a) : 0;
    return { type: 'parabola', equation: `y = ${a}x² ${b >= 0 ? '+' : '−'} ${Math.abs(b)}x ${c >= 0 ? '+' : '−'} ${Math.abs(c)}`, x_range: [vertexX - 10, vertexX + 10] };
  }
...
function validateMockFigures(mock) {
  ...
  const normalize = (question) => {
    const canonicalFigure = canonicalizeFigure(question);
    if (!canonicalFigure) return question;
    const visualVariant = String(question.questionId || 'question').replace(/[^a-zA-Z0-9_-]/g, '-');
    const figure = { ...canonicalFigure, visualVariant };
    ...
    const algebraTypes = ['line','line_chart','bar_chart','table','coordinate_shape','linear_function_graph'];
