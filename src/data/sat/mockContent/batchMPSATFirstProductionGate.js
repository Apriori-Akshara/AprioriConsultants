...
const TARGET = getBatchMProductionTarget('PSAT1');
const PRIOR_KEYS = ['SAT1','SAT2','SAT3','SAT4','SAT5','SAT6','SAT7','SAT8','SAT9','SAT10'];
const PRIOR_IDS = ['sat-series-a-mock-01','sat-series-a-mock-02','sat-series-a-mock-03','sat-series-a-mock-04','sat-series-a-mock-05','sat-series-a-mock-06','sat-series-a-mock-07','sat-series-a-mock-08','sat-series-a-mock-09','sat-series-a-mock-10'];
...
function assertCrossMockUniqueness(previousMocks, nextMock) {
  ...
  for (const question of nextMock.readingWriting || []) {
    const context = String(question.metadata?.contextKey || '').trim().toLowerCase();
    const prompt = String(question.prompt || '').trim().toLowerCase().replace(/\s+/g, ' ');
    if (context && previousRWContexts.has(context)) throw new Error(`Batch M PSAT1: R&W context reused: ${question.questionId}`);
    if (prompt && previousRWPrompts.has(prompt)) throw new Error(`Batch M PSAT1: R&W prompt reused: ${question.questionId}`);
  }
  for (const question of nextMock.math || []) {
    const application = String(question.metadata?.applicationFingerprint || '').trim().toLowerCase();
    if (application && previousApplications.has(application)) throw new Error(`Batch M PSAT1: Math application reused: ${question.questionId}`);
    const figureData = getFigureDataFingerprint(question);
    if (figureData && previousFigureData.has(figureData)) throw new Error(`Batch M PSAT1: figure data reused: ${question.questionId}`);
  }
...
function assertCanonicalRoundTrip(mock) {
  ...
  if (records.length !== 196) throw new Error(`Batch M PSAT1: expected 196 records, found ${records.length}`);
  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) throw new Error(`Batch M PSAT1: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
  }
  const restored = JSON.parse(JSON.stringify(records));
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) throw new Error('Batch M PSAT1: storage round-trip changed question IDs');
  return restored;
}

export function runBatchMPSATFirstProductionGate(previousMocks) {
  if (!TARGET) throw new Error('Batch M PSAT1: production target is missing');
  assertBatchMProductionOrder(PRIOR_KEYS);
  if (!Array.isArray(previousMocks) || previousMocks.length !== 10) throw new Error('Batch M PSAT1: accepted SAT Series A Mock 1 through Mock 10 baselines are required');
  if (previousMocks.some((mock, index) => mock?.testId !== PRIOR_IDS[index])) throw new Error('Batch M PSAT1: accepted Series A baselines are out of order');

  const stage1 = buildProductionMock({ testId: 'psat-mock-01', variant: 'psat-nmsqt', seed: 2001 });
  const withMath = { ...stage1, math: buildMathBank({ testId: 'psat-mock-01', variant: 'psat-nmsqt', assessmentNumber: 1, seed: 2001 }) };
...
export default runBatchMPSATFirstProductionGate;
