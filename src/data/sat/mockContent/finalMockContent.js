...
const balancedRobustnessChoices = [
  'It tests whether the first pattern remains across a wider range of settings.',
  'It tests whether the first pattern changes across a wider range of settings.',
  'It shows whether the original pattern remains under broader conditions.',
  'It examines whether the first pattern applies in every setting.'
];
...
function normalizeChoices(question) {
  ...
  else if (question.skill === 'Command of Evidence' && question.prompt.includes('narrower estimate')) choices = balancedRobustnessChoices;
...
export function finalizeMockQuestions(records) { return records.map((question, index) => convertSelectedMathItems(normalizeChoices(question), index)); }
export function buildMock({ testId, variant, section, module }) { return finalizeMockQuestions(buildBaseMock({ testId, variant, section, module })); }
