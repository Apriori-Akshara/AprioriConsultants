...
const TARGET = getBatchMProductionTarget('SAT20');
const PRIOR_KEYS = ['SAT1','SAT2','SAT3','SAT4','SAT5','SAT6','SAT7','SAT8','SAT9','SAT10','PSAT1','PSAT2','PSAT3','PSAT4','PSAT5','PSAT6','PSAT7','PSAT8','PSAT9','PSAT10','SAT11','SAT12','SAT13','SAT14','SAT15','SAT16','SAT17','SAT18','SAT19'];
const PRIOR_IDS = ['sat-series-a-mock-01','sat-series-a-mock-02','sat-series-a-mock-03','sat-series-a-mock-04','sat-series-a-mock-05','sat-series-a-mock-06','sat-series-a-mock-07','sat-series-a-mock-08','sat-series-a-mock-09','sat-series-a-mock-10','psat-mock-01','psat-mock-02','psat-mock-03','psat-mock-04','psat-mock-05','psat-mock-06','psat-mock-07','psat-mock-08','psat-mock-09','psat-mock-10','sat-series-b-mock-11','sat-series-b-mock-12','sat-series-b-mock-13','sat-series-b-mock-14','sat-series-b-mock-15','sat-series-b-mock-16','sat-series-b-mock-17','sat-series-b-mock-18','sat-series-b-mock-19'];
...
function assertCrossMockUniqueness(previousMocks, nextMock) {
  ...
  for (const question of nextMock.readingWriting || []) {
    const context = String(question.metadata?.contextKey || '').trim().toLowerCase();
    const prompt = String(question.prompt || '').trim().toLowerCase().replace(/\s+/g, ' ');
    if (context && previousRWContexts.has(context)) throw new Error(`Batch M SAT20: R&W context reused: ${question.questionId}`);
    if (prompt && previousRWPrompts.has(prompt)) throw new Error(`Batch M SAT20: R&W prompt reused: ${question.questionId}`);
  }
  for (const question of nextMock.math || []) {
    const application = String(question.metadata?.applicationFingerprint || '').trim().toLowerCase();
    if (application && previousApplications.has(application)) throw new Error(`Batch M SAT20: Math application reused: ${question.questionId}`);
    const figureData = getFigureDataFingerprint(question);
    if (figureData && previousFigureData.has(figureData)) throw new Error(`Batch M SAT20: figure data reused: ${question.questionId}`);
  }
...
function assertCanonicalRoundTrip(mock) {
  ...
  if (records.length !== 196) throw new Error(`Batch M SAT20: expected 196 records, found ${records.length}`);
  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) throw new Error(`Batch M SAT20: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
  }
  const restored = JSON.parse(JSON.stringify(records));
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) throw new Error('Batch M SAT20: storage round-trip changed question IDs');
  return restored;
}

export function runBatchMSAT20ProductionGate(previousMocks) {
  if (!TARGET) throw new Error('Batch M SAT20: production target is missing');
  assertBatchMProductionOrder(PRIOR_KEYS);
  if (!Array.isArray(previousMocks) || previousMocks.length !== 29) throw new Error('Batch M SAT20: accepted SAT1–10, PSAT1–10, and SAT11–19 baselines are required');
  if (previousMocks.some((mock, index) => mock?.testId !== PRIOR_IDS[index])) throw new Error('Batch M SAT20: accepted production baselines are out of order');

  const stage1 = buildProductionMock({ testId: 'sat-mock-20', variant: 'sat-series-b', seed: 1020 });
  const withMath = { ...stage1, math: buildMathBank({ testId: 'sat-mock-20', variant: 'sat-series-b', assessmentNumber: 20, seed: 1020 }) };
...
export default runBatchMSAT20ProductionGate;