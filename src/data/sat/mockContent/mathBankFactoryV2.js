...
function makeQuestion({ testId, variant, assessmentNumber, module, route, domain, difficulty, i, seed }) {
  ...
  if (domain === 'Algebra') {
    ...
    else if (form === 2) { const h = 3 + n % 13 + seed * 2; const k = 4 + Math.floor(n / 13) % 11 + seed * 3; skill = 'Quadratic functions and representations'; prompt = `For f(x) = (x − ${h})² + ${k}, what is the minimum value of f?`; answer = k; explanation = `The square is minimized at 0, so the minimum is ${k}.`; figure = FIG.quadratic(1, -2 * h, h * h + k); features = ['representation-shift','multi-step','strategic-choice']; }
    ...
    if (form === 0) { const base = 7 + n % 14 + seed * 2; const height = 6 + Math.floor(n / 14) % 13 + seed * 2; skill = 'Geometry and measurement'; prompt = `A triangle has base ${base} and height ${height}. What is its area?`; answer = base * height / 2; explanation = 'Use one-half times base times height.'; figure = FIG.geometry('triangle', { base, height }); features = ['multi-step']; }
    else if (form === 1) { const radius = 4 + n % 17 + seed * 2; skill = 'Circles'; prompt = `A circle has radius ${radius}. What is its area in terms of π?`; answer = `${radius * radius}π`; explanation = 'Area equals πr².'; figure = FIG.geometry('circle', { radius }); }
    else if (form === 2) { const x = 5 + n % 13 + seed * 2; const y = 6 + Math.floor(n / 13) % 14 + seed * 2; skill = 'Right triangles'; prompt = `A right triangle has legs ${x} and ${y}. What is its area?`; answer = x * y / 2; explanation = 'Area equals one-half the product of the perpendicular legs.'; figure = FIG.geometry('right-triangle', { x, y }); features = ['representation-shift','multi-step']; }
    else { const scale = 2 + n % 5 + seed; const area = 12 + Math.floor(n / 5) % 41 + seed * 4; skill = 'Similarity and scaling'; prompt = `Two similar figures have corresponding lengths in the ratio ${scale}:1. The smaller figure has area ${area}. What is the larger area?`; answer = area * scale * scale; explanation = 'Areas scale by the square of the length ratio.'; figure = FIG.geometry('similar-figures', { scale, area }); features = ['multi-step','constraint-inference']; }
  }
