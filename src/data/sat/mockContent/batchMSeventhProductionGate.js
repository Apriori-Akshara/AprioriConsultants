/**
 * Batch M — SAT Series A Mock 7 production gate.
 *
 * Mock 7 is accepted only after SAT Series A Mocks 1–6 have already passed.
...
const TARGET = getBatchMProductionTarget('SAT7');
...
function assertCrossMockUniqueness(previousMocks, nextMock) {
  ...
  for (const question of nextMock.readingWriting || []) {
    ...
    if (context && previousRWContexts.has(context)) {
      throw new Error(`Batch M SAT7: R&W context reused from an accepted earlier mock: ${question.questionId}`);
    }
    if (prompt && previousRWPrompts.has(prompt)) {
      throw new Error(`Batch M SAT7: R&W prompt reused from an accepted earlier mock: ${question.questionId}`);
    }
  }
  ...
  for (const question of nextMock.math || []) {
    ...
    if (application && previousApplications.has(application)) {
      throw new Error(`Batch M SAT7: Math application reused from an accepted earlier mock: ${question.questionId}`);
    }
    const figureData = getFigureDataFingerprint(question);
    if (figureData && previousFigureData.has(figureData)) {
      throw new Error(`Batch M SAT7: figure data reused from an accepted earlier mock: ${question.questionId}`);
    }
  }
...
function assertCanonicalRoundTrip(mock) {
  ...
  if (records.length !== 196) {
    throw new Error(`Batch M SAT7: expected 196 records, found ${records.length}`);
  }

  for (const record of records) {
    const result = validateSatQuestion(record);
    if (!result.valid) {
      throw new Error(`Batch M SAT7: canonical schema rejected ${record.questionId}: ${result.errors.join(' ')}`);
    }
  }
  ...
  if (JSON.stringify(records.map((record) => record.questionId)) !== JSON.stringify(restored.map((record) => record.questionId))) {
    throw new Error('Batch M SAT7: storage round-trip changed question IDs');
  }

  return restored;
}

export function runBatchMSeventhProductionGate(previousMocks) {
  if (!TARGET) throw new Error('Batch M SAT7: production target is missing');
  assertBatchMProductionOrder(['SAT1', 'SAT2', 'SAT3', 'SAT4', 'SAT5', 'SAT6']);

  if (!Array.isArray(previousMocks) || previousMocks.length !== 6) {
    throw new Error('Batch M SAT7: accepted Mock 1 through Mock 6 baselines are required');
  }
  ...
  if (previousMocks.some((mock, index) => mock?.testId !== expectedIds[index])) {
    throw new Error('Batch M SAT7: accepted Series A baselines are out of order');
  }

  const stage1 = buildProductionMock({ testId: 'sat-mock-07', variant: 'sat-series-a', seed: 1007 });
  const withMath = {
    ...stage1,
    math: buildMathBank({ testId: 'sat-mock-07', variant: 'sat-series-a', assessmentNumber: 7, seed: 1007 }),
  };
...
export default runBatchMSeventhProductionGate;
