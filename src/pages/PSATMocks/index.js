...
export async function getServerSideProps(context) {
  ...
  if (!accessState.authenticated) {
    return { redirect: { destination: getSatLoginUrl('/PSATMocks'), permanent: false } }
  }
...
function MockCard({ testNumber, attempt }) {
  const routeKey = `PSAT${testNumber}`
  const title = `PSAT/NMSQT Mock Test ${String(testNumber).padStart(2, '0')}`
  const label = `PSAT / NMSQT · MOCK ${String(testNumber).padStart(2, '0')}`
  const description = 'Original PSAT/NMSQT-style adaptive practice with timed modules, persistent progress, scoring, and reporting.'
...
export default function PSATMocks() {
  ...
  const completedCount = progress.completed?.filter((item) => String(item?.test_key || '').startsWith('PSAT')).length || 0
  const inProgressCount = (progress.attempts || []).filter((item) => item.status === 'in-progress' && String(item?.test_key || '').startsWith('PSAT')).length
  const bestAccuracy = useMemo(() => {
    const values = (progress.completed || [])
      .filter((item) => String(item?.test_key || '').startsWith('PSAT'))
  ...
  const attemptsByTestKey = useMemo(() => {
    ...
    for (const item of progress.attempts || []) {
      ...
      if (!key.startsWith('PSAT')) continue
      ...
      <div className={styles.container}>
        <section className={styles.hero}>
          <div>
            ...
            <h1>PSAT Mock Tests</h1>
            <p>Ten full-length PSAT/NMSQT-style adaptive practice forms with timed modules, persistent attempts, scoring, and a progress trail that carries into your student dashboard.</p>
          </div>
          <div className={styles.heroBadge}><span>10</span><small>Live Mocks</small></div>
        </section>

        <section className={styles.summaryGrid}>
          <div className={styles.summaryCard}><span className={styles.summaryLabel}>TESTS COMPLETED</span><strong>{completedCount}</strong><p>Completed PSAT attempts saved to your account</p></div>
          <div className={styles.summaryCard}><span className={styles.summaryLabel}>IN PROGRESS</span><strong>{inProgressCount}</strong><p>PSAT mocks with an active saved attempt</p></div>
          <div className={styles.summaryCard}><span className={styles.summaryLabel}>BEST ACCURACY</span><strong>{bestAccuracy}%</strong><p>Highest completed PSAT mock accuracy so far</p></div>
          <div className={styles.summaryCard}><span className={styles.summaryLabel}>FULL LIBRARY</span><strong>10</strong><p>10 PSAT/NMSQT mock forms</p></div>
        </section>
        ...
        <section className={styles.testGrid}>
          {tests.map((testNumber) => (
            <MockCard key={`PSAT${testNumber}`} testNumber={testNumber} attempt={attemptFor(`PSAT${testNumber}`)} />
          ))}
        </section>

        <div className={styles.dashboardFooterNote}>
          <p>All PSAT mock attempts are saved against the same verified student account and reflected in the Profile dashboard.</p>
