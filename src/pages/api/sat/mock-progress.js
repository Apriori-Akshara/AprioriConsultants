  ...
  try {
    ...
    if (req.method === "GET") {
      const result = await query(
        `SELECT id, test_key, status, current_section, current_module, current_question,
           ...
           FROM sat_mock_attempts WHERE user_id = $1 ORDER BY updated_at DESC`,
        [user.id]
      );
    ...
    if (action === "start") {
      const existing = await query(
        `SELECT * FROM sat_mock_attempts WHERE user_id = $1 AND test_key = $2 AND status = 'in-progress' ORDER BY updated_at DESC LIMIT 1`,
        [user.id, testKey]
      );
    ...
    if (action === "begin") {
      ...
      await query(
        `UPDATE sat_mock_attempts SET module_started_at = NOW(), module_deadline_at = $1, updated_at = NOW() WHERE id = $2 AND user_id = $3 AND status = 'in-progress'`,
    ...
    if (action === "answer") {
      ...
      await query(
        `UPDATE sat_mock_attempts SET answers = $1::jsonb, current_section = COALESCE($2, current_section), current_module = COALESCE($3, current_module), current_question = COALESCE($4, current_question), updated_at = NOW() WHERE id = $5 AND user_id = $6 AND status = 'in-progress'`,
    ...
    if (action === "advance") {
      ...
      if (section === "reading-writing") {
        ...
        const deadline = deadlineForModule(nextModule);
        await query(`UPDATE sat_mock_attempts SET current_section = 'math', current_module = 'module-1', current_question = 0, module_started_at = NOW(), module_deadline_at = $1, break_deadline_at = NULL, updated_at = NOW() WHERE id = $2 AND user_id = $3 AND status = 'in-progress'`, [deadline, attemptId, user.id]);
