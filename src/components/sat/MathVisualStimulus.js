  ...
  return (
    <div className={styles.figureBox} aria-label={title}>
      ...
      <svg viewBox="0 0 580 250" role="img">
        <line x1="40" y1="210" x2="550" y2="210" className={styles.axis} />
        <line x1="40" y1="25" x2="40" y2="210" className={styles.axis} />
        {polyline && <polyline points={polyline} className={styles.graphLine} fill="none" />}
        {scaled.map(([x, y], index) => <circle key={`${x}-${index}`} cx={x} cy={y} r="5" className={styles.graphPoint} />)}
  ...
  return (
    <div className={styles.figureBox} aria-label={title}>
      ...
      <svg viewBox="0 0 580 250" role="img">
        <line x1="40" y1="210" x2="550" y2="210" className={styles.axis} />
        <line x1="40" y1="25" x2="40" y2="210" className={styles.axis} />
        {scaled.map(([x, y], index) => <circle key={`${x}-${index}`} cx={x} cy={y} r="6" className={styles.scatterPoint} />)}
        <text x="548" y="230">x</text><text x="22" y="32">y</text>
      </svg>
    </div>
  );
...
function QuadraticFigure({ figure, title }) {
  const a = Number(figure.a || 1);
  const b = Number(figure.b || 0);
  const c = Number(figure.c || 0);
  ...
  return (
    <div className={styles.figureBox} aria-label={title}>
      ...
      <svg viewBox="0 0 580 250" role="img">
        <line x1="40" y1="210" x2="550" y2="210" className={styles.axis} />
        <line x1="290" y1="25" x2="290" y2="210" className={styles.axis} />
...
function GeometryFigure({ figure, title }) {
  ...
  if (shape === "rectangle") {
    ...
    const height = Number(values.height || 5);
  ...
  const base = Number(values.base || 8);
  const height = Number(values.height || 5);
