...
export default function ThreeDSolidFigure({ figure, title }) {
  ...
  if (type === "cylinder") {
    ...
    return <div className={styles.figureBox} aria-label={title}>
      ...
      <svg viewBox="0 0 580 250" role="img">
        ...
        {dims.filter((item) => !/^(radius|r|height|h)\s*=/.test(item)).map((item, index) => <MathSvgText key={index} x="220" y={225 + index * 16}>{item}</MathSvgText>)}
      </svg>
    </div>;
  }
  ...
  if (type === "sphere") {
    ...
    return <div className={styles.figureBox} aria-label={title}>
      ...
      <svg viewBox="0 0 580 250" role="img">
        ...
        <MathSvgText x="300" y="115">{labelValue(labels, "radius") || `r = ${radius}`}</MathSvgText>
      </svg>
    </div>;
  }
  ...
  if (type === "cone") {
    ...
    return <div className={styles.figureBox} aria-label={title}>
      ...
      <svg viewBox="0 0 580 250" role="img">
        ...
        <MathSvgText x="300" y="218">{labelValue(labels, "radius") || `r = ${radius}`}</MathSvgText>
      </svg>
    </div>;
  }
  ...
  return <div className={styles.figureBox} aria-label={title}>
    ...
    <svg viewBox="0 0 580 250" role="img">
      ...
      {labelValue(labels, "front") && <MathSvgText x="250" y="245">{labelValue(labels, "front")}</MathSvgText>}
    </svg>
  </div>;
}
