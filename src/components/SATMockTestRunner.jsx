...
import { getModuleForRoute } from "../lib/sat/productionAdaptiveMockEngine";
import MathVisualStimulus from "./sat/MathVisualStimulus";
...
export default function SATMockTest({ test }) {
  ...
  return <div className={styles.page}>
    ...
    {tool ? <div className={styles.overlay} onClick={() => setTool(null)}><div className={tool === "navigator" ? styles.drawer : tool === "calculator" ? styles.calculatorPanel : tool === "reference" ? styles.referencePanel : styles.notesPanel} onClick={(event) => event.stopPropagation()}>
      ...
      </div>: tool === "reference" ? <div className={styles.referenceGrid}><div><strong>Triangle</strong><MathText as="p">A = ½bh</MathText></div><div><strong>Circle</strong><MathText as="p">A = πr²</MathText></div><div><strong>Pythagorean theorem</strong><MathText as="p">a² + b² = c²</MathText></div><div><strong>Coordinate geometry</strong><MathText as="p">Use the coordinate-plane relationships needed by the question.</MathText></div></div> : <textarea value={notes[question?.questionId] || ""} onChange={(event) => setNotes((current) => ({ ...current, [question.questionId]: event.target.value }))} onBlur={(event) => saveNote(event.target.value)} placeholder="Write a note for this attempt…" />}
    </div></div> : null}
  </div>;
}